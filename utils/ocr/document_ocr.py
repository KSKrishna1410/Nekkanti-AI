...
class DocumentOCR:
    ...
    def extract_tables_from_pdf(self, pdf_path=None, save_to_excel=True):
        ...
        try:
            ...
            pdf = Img2TablePDF(pdf_path,detect_rotation=True)
            ...
            with ThreadPoolExecutor(max_workers=num_processes) as executor:
                ...
                # Sort results by page number and collect tables with robust error handling
                results.sort(key=lambda x: x[0] if isinstance(x, tuple) and len(x) >= 2 else 0)  # Sort by page number
                for result in results:
                    try:
                        if isinstance(result, tuple) and len(result) >= 2:
                            _, page_tables = result
                            if isinstance(page_tables, list):
                                all_tables.extend(page_tables)
                            else:
                                print(f"⚠️ Unexpected page_tables format: {type(page_tables)}")
                        else:
                            print(f"⚠️ Unexpected result format: {type(result)}")
                    except Exception as e:
                        print(f"⚠️ Error processing parallel result: {e}")
                        continue
