...
class DocumentTableExtractor:
    ...
    def _clean_dataframe_text(self, df):
        ...
        def clean_text(text):
            ...
            # Handle complex nested structures from img2table
            if isinstance(text, (list, tuple, np.ndarray)):
                try:
                    # Convert nested structures to string by joining
                    if len(text) > 0:
        ...
        for col in cleaned_df.columns:
            try:
                ...
                print(f"⚠️ Warning: Error cleaning column {col}: {e}")
    ...
    def _post_process_table(self, df):
        ...
        Handles cases where a single transaction spans multiple rows.
        """
    ...
    def _merge_row_into_target(self, df, source_idx, target_idx):
        ...
        """
        # Get non-empty cells from source row - FIXED to prevent Series corruption
        source_row = df.iloc[source_idx].copy()
        
        # Safely process each cell individually to avoid Series corruption
        processed_values = []
        for cell in source_row:
            if pd.isna(cell) or cell is None:
                processed_values.append(pd.NA)
            else:
                # Handle complex structures like lists/tuples 
                if isinstance(cell, (list, tuple, np.ndarray)):
                    try:
                        if len(cell) > 0:
                            cell_str = ' '.join(str(item) for item in cell if item is not None)
                        else:
                            cell_str = ''
                    except:
                        cell_str = str(cell)
                ...
                if cell_str in ['None', 'nan', '', 'NaN', 'NA']:
                    processed_values.append(pd.NA)
                else:
                    processed_values.append(cell_str)
        
        for col_idx, value in enumerate(processed_values):
    ...
    def _extract_tables_with_img2table(self, pdf_path):
        ...
        try:
            ...
            # Standard settings for all banks (removed special Canara Bank handling)
            try:
                ...
                for table_info in extracted_tables:
                    try:
                        ...
                        if isinstance(table_info, dict):
                            # Normal case: dictionary with table data
                            df = table_info['dataframe'].copy()
                            page_number = table_info.get('page_number', 1)
                            table_index = table_info.get('table_index', 0)
                        elif isinstance(table_info, (list, tuple)) and len(table_info) >= 2:
                            # Handle parallel processing results: [page_number, table_data_list]
                            page_number, table_data_list = table_info
                            if isinstance(table_data_list, list) and len(table_data_list) > 0:
                                # Take the first table from the list
                                table_data = table_data_list[0]
                                if isinstance(table_data, dict):
                                    df = table_data['dataframe'].copy()
                                    page_number = table_data.get('page_number', page_number)
                                    table_index = table_data.get('table_index', 0)
                                else:
                                    print(f"⚠️ Unexpected table data format: {type(table_data)}")
                                    continue
                            else:
                                print(f"⚠️ Empty table data list")
                                continue
                        else:
                            print(f"⚠️ Unexpected table_info format: {type(table_info)}")
                            continue
                        
                        # Clean Unicode artifacts and OCR noise
                        df = self._clean_dataframe_text(df)
                        
                        print(f"📊 Processing table from page {page_number}: {df.shape[0]}×{df.shape[1]}")
                        
                        if df.empty:
                            print(f"⚠️ Table on page {page_number} is empty from NekkantiOCR")
                            continue
                        
                        # Create consistent sheet name
                        sheet_name = f"page{page_number}_table{table_index}"[:31]
                        ...
                        print(f"✅ Table {table_index} on page {page_number}: {df.shape[0]} rows × {df.shape[1]} columns")
                        
                    except Exception as e:
                        print(f"⚠️ Error processing table: {e}")
                        print(f"⚠️ Table info type: {type(table_info)}")
                        continue
