...
class BankStatementHeaderExtractor:
            ...
            for pattern, field_type in range_patterns:
                if re.search(pattern, text_lower):
                    if field_type == 'both':
                        ...
                        if len(dates) >= 2:
                            # Match old format: one field for the whole range
                            key_matches.append({
                                'field_name': 'Statement Date From',
                                'matched_text': text,
                                'keyword': 'date range pattern',
                                'bbox': bbox,
                                'data_type': 'String', # Treat as string to keep full text
                                'field_type': 'Header',
                                'confidence': confidence,
                                'match_score': 0.95,
                                'text_index': text_idx
    ...
    def _extract_value_from_smart_match(self, key_match: Dict) -> Optional[str]:
        ...
        if field_name in ['Statement Date', 'Statement Date From', 'Statement Date To']:
            if 'date pattern' in keyword:
                ...
                # To match old format, extract the full date range string
                if field_name == 'Statement Date From':
                    # Find the part of the string that contains the date range
                    range_match = re.search(r'(\d{1,2}[/-]\d{1,2}[/-]\d{2,4}\s+to\s+\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', text, re.IGNORECASE)
                    if range_match:
                        extracted = range_match.group(1)
                        # Prepend colon if it exists in the original text before the range
                        if re.search(r':\s*' + re.escape(extracted), text, re.IGNORECASE):
                             return ": " + extracted
                        return extracted
                ...
                dates = re.findall(r'\d{1,2}[/-]\d{1,2}[/-]\d{2,4}', text)
                if len(dates) >= 2:
                    if field_name == 'Statement Date To': # This won't be called if my first change works
                        extracted = dates[-1]
                    else:
                        extracted = dates[-1]
                    return extracted
    ...
    def _validate_and_score_value(self, value_text: str, data_type: str, field_name: str) -> Tuple[bool, float]:
        ...
        wrong_values = [
            ...
            # 'branch phone', 
            'phone', 'email', 'website', 'address', 'pin', 'code'
        ]
    ...
    def _format_value_for_output(self, value: Any, data_type: str) -> Any:
        """Format the extracted value for the final JSON output."""
        if value is None:
            return ""
    ...
    def _map_method_name(self, method: str) -> str:
        """Map internal method names to the desired output format."""
        mapping = {
            'right_aligned': 'right_aligned_pair',
            'bottom_aligned': 'bottom_aligned_pair',
            'smart_pattern': 'regex_match',
            'ifsc_lookup': 'ifsc_lookup',
            'keyword_separator': 'keyword_match',
            'keyword_masked': 'keyword_match',
            'keyword_fragmented': 'keyword_match',
            'ifsc_length_based': 'ifsc_length_match',
            'ifsc_length_based_fragmented': 'ifsc_length_match',
            'regex_pattern': 'regex_match'
        }
        return mapping.get(method, method)

    def extract_headers(self, pdf_path: str) -> List[Dict[str, Any]]:
        """
        ...
        Returns:
            List of dictionaries with extracted header information
        """
        ...
        try:
            ...
            used_values = set()
            found_fields = set()
            ifsc_details = {'code': None, 'bbox': None}
            ...
            for key_match in key_matches:
                ...
                if best_value:
                    formatted_value = self._format_value_for_output(best_value['text'], key_match['data_type'])
                    
                    header_item = {
                        "key": field_name,
                        "value": formatted_value,
                        "method": self._map_method_name(best_value['method']),
                        "key_bbox": key_match['bbox'],
                        "value_bbox": best_value['bbox'],
                        "doc_text": key_match['matched_text'],
                        "closest_distance": best_value['distance']
                    }
                    final_header_list.append(header_item)
                    
                    used_values.add(best_value['index'])
                    found_fields.add(field_name)

                    if field_name == 'IFSC Code' and formatted_value:
                        ifsc_details['code'] = formatted_value
                        ifsc_details['bbox'] = best_value['bbox']

                    print(f"✅ {field_name}: '{formatted_value}' (method: {best_value['method']})")

            # Fallback for Account Number
            if 'Account Number' not in found_fields:
                print("🔄 Account Number not found, trying fallbacks...")
                account_info = self._find_account_number_near_labels(text_results) or \
                               self._find_account_number_with_ifsc_length(text_results)
                if account_info:
                    final_header_list.append({
                        "key": "Account Number",
                        "value": account_info['text'],
                        "method": self._map_method_name(account_info['method']),
                        "key_bbox": account_info.get('label_bbox', account_info['bbox']),
                        "value_bbox": account_info['bbox'],
                        "doc_text": account_info.get('label_text', account_info['original_text']),
                        "closest_distance": account_info.get('distance', 0.0)
                    })
                    found_fields.add('Account Number')
                    print(f"   ✅ Account Number found with fallback: {account_info['text']}")

            # Enhance with IFSC lookup
            if ifsc_details['code']:
                print(f"🔍 Looking up bank details for IFSC: {ifsc_details['code']}")
                bank_details = self._lookup_bank_details_by_ifsc(ifsc_details['code'])
                if bank_details:
                    for key, value in bank_details.items():
                        if value and key not in found_fields:
                            final_header_list.append({
                                "key": key,
                                "value": value,
                                "method": "ifsc_lookup",
                                "key_bbox": ifsc_details['bbox'],
                                "value_bbox": ifsc_details['bbox'],
                                "doc_text": ifsc_details['code'],
                                "closest_distance": 0.0
                            })
                            found_fields.add(key)
                            print(f"   ✅ Added via IFSC lookup: {key}: {value}")

            output_file = os.path.join(self.output_dir, f"{os.path.splitext(os.path.basename(pdf_path))[0]}_headers.json")
            with open(output_file, 'w') as f:
                json.dump(final_header_list, f, indent=2)
            print(f"💾 Saved {len(final_header_list)} final headers to: {output_file}")
    ...
    def extract_headers_batch(self, pdf_files: List[str]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for pdf_file in pdf_files:
            if os.path.exists(pdf_file):
                ...
                print(f"✅ Success: {len(headers)} header fields extracted" if headers else "❌ Failed to extract headers")
            else:
    ...
    def _find_account_number_with_regex(self, text_results: List[Tuple]) -> Optional[Dict]:
        ...
        for text_idx, (text, bbox, confidence) in enumerate(text_results):
            ...
            if re.match(account_pattern, cleaned_text):
                ...
                if self._validate_account_number_format(cleaned_text):
    ...
    def _find_account_number_near_labels(self, text_results: List[Tuple]) -> Optional[Dict]:
        ...
        Uses keywords from bankstmt_allkeys.csv for better accuracy.
        ...
        Returns:
            Dict with account number info or None if not found
        """
        print("🔍 Trying enhanced account detection using keywords...")
        
        # Get account number keywords from bankstmt_allkeys.csv
        account_keywords = [
            keyword_info['key'].lower() 
            for keyword_info in self.header_mappings.get('Account Number', [])
        ]
        
        # Add variations of keywords for better matching
        account_keywords.extend([
            'a/c', 'account', 'savings account', 'current account',
        ...
        for text_idx, (text, bbox, confidence) in enumerate(text_results):
            ...
            # Check if this text contains an account keyword
            ...
            if matched_keyword:
                print(f"   📍 Found account keyword: '{matched_keyword}' in '{text}'")
                
                # Look for account number in this text and next few elements
                ...
                for search_idx in search_range:
                    ...
                    # Skip if the text is too long (likely a transaction)
                    if len(search_text) > 100:
                        continue
                        
                    # Skip if it contains transaction indicators
                    transaction_indicators = [
                        'upi', 'neft', 'rtgs', 'imps', 'transfer', 'payment',
                    ...
                    for separator in separators:
                        if separator in search_text:
                            parts = search_text.split(separator)
                            for part in parts[1:]:  # Look in parts after separator
                    ...
                    if re.match(r'^[\d\s]+$', search_text):
                        ...
                        if self._validate_account_number_format(clean_number):
                            ...
                            print(f"      ✅ Found fragmented account: '{clean_number}' (keyword: {matched_keyword})")
        
        if not potential_accounts:
            print("   ❌ No account numbers found using keywords")
            return None
    ...
    def _validate_account_number_format(self, number: str) -> bool:
        """
        Validate account number format with bank-specific rules.
        
        Args:
            number: Account number to validate
            
        Returns:
            bool: Whether the number follows valid account format
        """
        if not number:
            return False
            
        # Clean the number
        clean_number = re.sub(r'[^\dXx]', '', number)
        
        # Validate masked account numbers
        if 'X' in number.upper():
            if not re.match(r'^[Xx]+\d{3,8}$', number, re.IGNORECASE):
                return False
            return True
            
        # Basic validation for regular account numbers
        if not clean_number.isdigit():
            return False
            
        # Length validation (most Indian bank accounts are 9-18 digits)
        ...
        for text_idx, (text, bbox, confidence) in enumerate(text_results):
            ...
            if re.match(r'^[\d\s]+$', text.strip()) and len(text.strip()) > 8:
                ...
                if min_length <= len(fragmented_number) <= max_length:
                    if self._validate_account_number_format(fragmented_number):
